...
struct ClassCueHomeProvider: TimelineProvider {
    ...
    private var sampleSnapshot: ClassCueWidgetSnapshot {
        let now = Date()
...
struct ClassCueHomeEntryView: View {
    ...
    private func compactHero(title: String, block: ClassCueWidgetSnapshot.BlockSummary, timerDate: Date) -> some View {
    ...
    private func compactNextRow(_ block: ClassCueWidgetSnapshot.BlockSummary) -> some View {
        let accent = accentColor(for: block)
    ...
    private func progressBar(for block: ClassCueWidgetSnapshot.BlockSummary) -> some View {
    ...
    private var emptyState: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Open ClassCue for tomorrow’s schedule, tasks, and supports.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .lineLimit(2)
    ...
    private func blockMetaText(_ block: ClassCueWidgetSnapshot.BlockSummary) -> String {
    ...
    private func accentColor(for block: ClassCueWidgetSnapshot.BlockSummary?) -> Color {
...
#Preview(as: .systemSmall) {
    ...
    ClassCueHomeEntry(
        date: .now,
        snapshot: ClassCueWidgetSnapshot(
            updatedAt: .now,
            current: .init(
                className: "Math",
                room: "204",
